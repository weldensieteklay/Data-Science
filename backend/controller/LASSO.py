...
#         mse = np.mean((lr_model.predict(test_data.drop(columns=[endogenous_variable])) - test_data[endogenous_variable])**2)
#         mean = lr_model.coef_

#         results_dict = [
#             {"field_name": "constant", "mean": f"{lr_model.intercept_:.3f}"}
#         ] + [
...
def run_time_series_lasso_model(data):
    try:
        ...
        mse = int(np.round(np.mean((lr_model.predict(test_data.drop(columns=[endogenous_variable])) - test_data[endogenous_variable])**2)))
